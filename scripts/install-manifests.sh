...
for i in {1..30}; do
    CSI_READY=$(kubectl get pods -n kube-system -l app=ebs-csi-controller --no-headers 2>/dev/null | grep -c "Running" || echo "0")
...
echo "  - From inside cluster: registry.registry.svc.cluster.local:5000"
echo "  - From nodes:          localhost:30500"
echo "  - From outside:        ${DB_IP}:30500"
echo ""
echo "To push images from outside the cluster:"
echo "  docker tag myimage:latest ${DB_IP}:30500/myimage:latest"
echo "  docker push ${DB_IP}:30500/myimage:latest"
echo ""
